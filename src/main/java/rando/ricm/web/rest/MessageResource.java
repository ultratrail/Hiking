...
public class MessageResource {
    ...
    @GetMapping("/messages/yourmessages/{id}")
    @Timed
    public ResponseEntity<List<Message>> getAllMessagesByHikerAndHike(Pageable pageable, @PathVariable Long id) {
        log.debug("REST request to get a page of Messages");
        Optional <String> userstr = SecurityUtils.getCurrentUserLogin();
    	Optional<User> user = userRepository.findOneByLogin(userstr.get());
    	Hiker hiker = hikerRepository.findOneByUser(user.get());
    	Hike hike = hikeRepository.findOneById(id);
        Page<Message> page = messageService.findAllBySenderAndHike(pageable, hiker, hike);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/messages/yourmessages");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
