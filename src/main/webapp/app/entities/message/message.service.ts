...
export class MessageService {
    ...
    private resourceUrlyourmessages =  SERVER_API_URL + 'api/messages/yourmessages';
    ...
    search(id: number): Observable<HttpResponse<Message[]>> {
        // const options = createRequestOption(req);
        return this.http.get<Message[]>(`${this.resourceUrlyourmessages}/${id}`, { observe: 'response' })
            .map((res: HttpResponse<Message[]>) => this.convertArrayResponse(res));
    }
    ...
    private convertItemFromServer(message: Message): Message {
        ...
        copy.datetime = this.dateUtils
            .convertDateTimeFromServer(message.datetime);
        return copy;
    }
